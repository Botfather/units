// RDL formatter used by the VS Code extension (CommonJS).
// This duplicates the parser + printer to avoid runtime deps.

function parseRDL(input) {
...
const PRINT_WIDTH = Number(process.env.RDL_PRINT_WIDTH || 100);
...
function formatRDL(source) {
  const ast = parseRDL(source);
...
function formatRDLRange(source, startOffset, endOffset) {
  const ast = parseRDL(source);
  const target = findSmallestNode(ast, startOffset, endOffset);
  if (!target) {
    return { formatted: formatRDL(source), start: 0, end: source.length };
  }
...
module.exports = { formatRDL, formatRDLRange };
