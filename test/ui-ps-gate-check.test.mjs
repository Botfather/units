...
test("evaluateGates passes when payload satisfies thresholds", () => {
  const payload = {
    summary: {
      ...
      avg_compiled_token_reduction: 0.12,
      avg_agent_tree_token_reduction: 0.2,
  ...
  const gates = {
    ...
    min_avg_compiled_token_reduction: 0.1,
    min_avg_agent_tree_token_reduction: 0.1,
...
test("evaluateGates fails with readable failure messages", () => {
  const payload = {
    summary: {
      ...
      avg_compiled_token_reduction: 0,
      avg_agent_tree_token_reduction: 0,
      avg_total_score: 0.6,
...
test("runGateCheck reads payload and gates from provided paths", async () => {
  ...
  await fs.writeFile(resultPath, `${JSON.stringify({
    summary: {
      ...
      avg_compiled_token_reduction: 0.18,
      avg_agent_tree_token_reduction: 0.22,
      avg_total_score: 1,
      avg_delta_from_baseline: 0.1,
    },
    library: {
